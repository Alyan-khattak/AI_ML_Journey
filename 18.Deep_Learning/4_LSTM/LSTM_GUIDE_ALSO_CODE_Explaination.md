# LSTM — Complete Guide
### From Gates to Next Word Prediction
...
Simple RNN has one big weakness — it **forgets** long-term context.

```
Sentence: "Naruto grew up in Konoha. He trained hard. He became the ___"

Simple RNN forgets "Naruto" by the time it reaches "___"
```

This is called the **Vanishing Gradient Problem** — gradients shrink to near-zero as they travel back through many time steps during training. The model stops learning from distant words.
...
It adds a **memory system** — it can choose what to remember, what to forget, and what to output at each step.

```
Simple RNN :  one state  → forgets early context
LSTM       :  two states → cell state (long memory) + hidden state (short memory)
```

---

## Visual Structure of LSTM
...
```
                    ...
                    │                                                   │
  C(t-1) ─────────►│──────────────────────────────────────────────►──►│─── C(t)
  (prev cell state) │     ↑ forget  ↑ add new       ↑ scale            │ (new cell state)
                    │     │         │                │                  │
                    │   [FORGET]  [INPUT]         [OUTPUT]             │
                    │   GATE      GATE             GATE                │
                    │     │         │                │                  │
...
Two things travel through time at every step:

```
C(t) = Cell State   → long-term memory (the "conveyor belt")
h(t) = Hidden State → short-term memory (what we output)
```

---

# PART 2 — The 4 Gates Explained

Every LSTM cell has 4 operations. Three are gates (they control flow using sigmoid 0→1). One creates candidate values using tanh.

---

## Gate 1 — Forget Gate

**Question it answers:** "What should I throw away from long-term memory?"

```
Input  : h(t-1) and x(t)
Formula: f(t) = sigmoid( W_f · [h(t-1), x(t)] + b_f )
Output : number between 0 and 1 for each memory slot
```

```
f(t) = 0.0  →  completely forget this
f(t) = 1.0  →  completely keep this
f(t) = 0.6  →  keep 60% of this
```

**Example:**

```
Reading: "Naruto trained hard. Sasuke left the village."

After "Sasuke left the village" →
Forget gate: forget that Sasuke was present → f ≈ 0.1
             keep that Naruto is the main character → f ≈ 0.9
```

```
Cell state update: C = f(t) × C(t-1)
                       ↑
                  old memory × forget weights
```
...
**Question it answers:** "What new information should I write into long-term memory?"

Two operations happen together:

```
i(t) = sigmoid( W_i · [h(t-1), x(t)] + b_i )   ← how much to write
g(t) = tanh(    W_g · [h(t-1), x(t)] + b_g )   ← what to write (-1 to 1)
```

```
i(t)  →  0 to 1   (gate: how much of new info to add)
g(t)  →  -1 to 1  (candidate: what the new info actually is)
```

**Combined:**

```
New memory to add = i(t) × g(t)
```

**Example:**

```
Reading: "Naruto became Hokage"

Input gate decides:
  i ≈ 0.9  →  this is important, write most of it
  g ≈ 0.8  →  "Hokage" is strongly positive new info

New memory added = 0.9 × 0.8 = 0.72 units of "Hokage info"
```
...
After forget gate and input gate:
...
```
C(t-1) = [0.9, 0.2, 0.7, ...]   ← old memory
f(t)   = [0.9, 0.1, 0.8, ...]   ← forget filter
i×g    = [0.0, 0.72, 0.1, ...]  ← new info

C(t)   = [0.81, 0.09+0.72, 0.56+0.1, ...]
       = [0.81, 0.81,       0.66,     ...]  ← updated memory
```

This is the **conveyor belt** — long-term memory flows through with selective updates.

---

## Gate 3 — Output Gate

**Question it answers:** "What part of memory should I output right now?"
...
```
tanh(C(t))  →  squish cell state to -1 to 1 (what we know)
o(t)        →  how much of it to output (0 to 1)
h(t)        →  the actual output / new hidden state
```

**Example:**

```
Predicting next word for: "Naruto became ___"

Cell state C(t) contains: Naruto, trained, village, Hokage info
Output gate: "right now I only need the role-related memory"
o(t) ≈ high for "Hokage slot", low for other slots
h(t) → points strongly toward "Hokage"
```

---

## Summary — All 4 Operations

```
┌──────────────────┬─────────────────────────────────────────────────┐
│ Gate             │ Job                                              │
├──────────────────┼─────────────────────────────────────────────────┤
│ Forget Gate f(t) │ What to delete from long-term memory            │
│ Input Gate  i(t) │ How much new info to write                      │
│ Candidate   g(t) │ What the new info actually is                   │
│ Output Gate o(t) │ What to output from current memory              │
└──────────────────┴─────────────────────────────────────────────────┘

C(t) = f × C(t-1)  +  i × g       ← long-term memory update
h(t) = o × tanh(C(t))             ← output / short-term memory
```
